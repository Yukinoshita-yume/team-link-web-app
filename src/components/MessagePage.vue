<template>
  <div class="page">
    ...
    <nav class="nav">
      ...
      <span class="nav-title">系统通知</span>
      <div style="width:72px"></div>
    </nav>

    <div class="content">
      <!-- 未读 -->
      <template v-if="pendingMessages.length > 0">
        <div class="section-label">
          <span class="label-dot dot-purple"></span>未读通知
        ...
        <div class="msg-card pending" v-for="(msg, i) in pendingMessages" :key="i" @click="readMessage(i)">
          <div class="msg-avatar" :style="{ background: randomColor(msg.userName) }">{{ initial(msg.userName) }}</div>
          <div class="msg-body">
            <div class="msg-top">
              <span class="msg-name">{{ msg.userName }}</span>
              <span class="msg-time">{{ formatTime(msg.messageCreatedTime) }}</span>
            </div>
            <div class="msg-competition">{{ msg.title || '系统消息' }}</div>
      ...
      <template v-if="historyMessages.length > 0">
        <div class="section-label" style="margin-top: 8px">
          <span class="label-dot dot-gray"></span>历史通知
        </div>
        <div class="msg-card history" v-for="(msg, i) in historyMessages" :key="'h'+i">
          <div class="msg-avatar" :style="{ background: randomColor(msg.userName || '?') }">{{ initial(msg.userName || '?') }}</div>
          <div class="msg-body">
            <div class="msg-top">
              <span class="msg-name">{{ msg.userName }}</span>
      ...
      <div class="empty-state" v-if="pendingMessages.length === 0 && historyMessages.length === 0">
        ...
        <div class="empty-title">暂无系统通知</div>
      </div>
    </div>
  </div>
</template>
...
const randomColor = (s) => COLORS[(s?.charCodeAt(0) || 0) % COLORS.length]
...
async function loadMessages() {
  try {
    const [r1, r2] = await Promise.all([
      memberMessageApi({ userId: userId.value, isRead: false }),
      memberMessageApi({ userId: userId.value, isRead: true  }),
    ])
    if (r1.code === 0) pendingMessages.value = r1.data
    if (r2.code === 0) historyMessages.value = r2.data
  } catch (e) { console.error(e) }
}

async function readMessage(i) {
  try {
    const res = await readApi({ messageId: pendingMessages.value[i].messageId })
    if (res.code === 0) {
      historyMessages.value.unshift(pendingMessages.value[i])
      pendingMessages.value.splice(i, 1)
    }
  } catch (e) { console.error(e) }
}
...
.nav { position: sticky; top: 0; z-index: 50; display: flex; align-items: center; justify-content: space-between; padding: 14px 20px; background: rgba(255,255,255,0.85); backdrop-filter: blur(16px); border-bottom: 1px solid rgba(0,0,0,0.05); }
.back-btn { display: flex; align-items: center; gap: 6px; padding: 7px 16px; background: rgba(255,255,255,0.8); border: 1px solid rgba(0,0,0,0.08); border-radius: 20px; font-size: 14px; color: #555; cursor: pointer; transition: all 0.2s; }
.back-btn:hover { background: white; transform: translateY(-1px); }
.nav-title { position: absolute; left: 50%; transform: translateX(-50%); font-size: 20px; font-weight: 800; color: #1a1028; pointer-events: none; }
.content { position: relative; z-index: 1; max-width: 680px; margin: 0 auto; padding: 16px 16px 60px; }
