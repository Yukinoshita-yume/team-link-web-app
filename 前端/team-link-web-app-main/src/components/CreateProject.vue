        ...
        <div class="field">
          ...
          <div class="tags-wrap">
            <div class="tag-chip" v-for="(tag, i) in tags" :key="i">
              <input v-model="tags[i]" type="text" :placeholder="'标签 ' + (i+1)" @input="handleTagInput(i)"/>
...
const title = ref(''), maxParticipants = ref(null), schoolRequirements = ref('none')
const schoolName = ref(''), deadline = ref(''), tags = ref(['']), competitionDetails = ref('')
...
const handleTagInput = (i) => {
  if (tags.value[i]?.trim() === '' && tags.value.length > 1) tags.value.splice(i, 1)
  if (new Blob([tags.value[i]]).size > 15) tags.value[i] = tags.value[i].slice(0, -1)
}
const handleSubmit = async () => {
  ...
  try {
    ...
    const res = await createCompetitionApi({
      title: title.value, maxParticipants: maxParticipants.value,
      schoolRequirements: schoolRequirements.value === 'specific' ? schoolName.value : '',
      deadline: deadline.value + ' 23:59:59',
      tag1: tags.value[0] || null, tag2: tags.value[1] || null,
      tag3: tags.value[2] || null, tag4: tags.value[3] || null, tag5: tags.value[4] || null,
      competitionDetails: competitionDetails.value, userId: localUser.value.userId
    })
...
onMounted(() => { if (localUser.value.userId === -1) router.replace('/login') })
</script>
...
.nav { position: sticky; top: 0; z-index: 10; display: flex; align-items: center; justify-content: space-between; padding: 14px 20px; background: rgba(255,255,255,0.85); backdrop-filter: blur(16px); border-bottom: 1px solid rgba(0,0,0,0.05); }
/* 替换原有的 .back-btn 样式 */
.nav-back {
  display: flex;
  align-items: center;
...
.nav-title { font-size: 22px; font-weight: 700; color: #1a1028; }
.submit-btn-nav { padding: 8px 18px; background: linear-gradient(135deg, #8b5cf6, #6d28d9); color: white; border: none; border-radius: 20px; font-size: 14px; font-weight: 600; cursor: pointer; box-shadow: 0 3px 12px rgba(109,40,217,0.25); transition: all 0.2s; }
...
.form-card { background: rgba(255,255,255,0.78); border: 1px solid rgba(255,255,255,0.9); border-radius: 20px; padding: 24px; backdrop-filter: blur(12px); box-shadow: 0 2px 16px rgba(100,80,200,0.06); display: flex; flex-direction: column; gap: 20px; }
...
input, textarea {
  padding: 11px 14px; background: rgba(245,243,255,0.6);
  border: 1.5px solid rgba(139,92,246,0.12); border-radius: 10px;
  font-size: 14px; color: #333; outline: none; transition: all 0.2s; font-family: inherit;
}
input:focus, textarea:focus { border-color: rgba(139,92,246,0.45); background: white; box-shadow: 0 0 0 3px rgba(139,92,246,0.08); }
input::placeholder, textarea::placeholder { color: #ccc; }
textarea { min-height: 120px; resize: vertical; }
.radio-group { display: flex; gap: 10px; }
.radio-item { display: flex; align-items: center; gap: 7px; padding: 9px 16px; background: rgba(0,0,0,0.03); border: 1.5px solid rgba(0,0,0,0.07); border-radius: 10px; cursor: pointer; font-size: 14px; color: #555; font-weight: 400; transition: all 0.2s; }
...
.tag-chip { display: flex; align-items: center; background: rgba(139,92,246,0.07); border: 1.5px solid rgba(139,92,246,0.15); border-radius: 10px; overflow: hidden; }
.tag-chip input { border: none; background: transparent; padding: 8px 10px; font-size: 13px; width: 90px; outline: none; }
.tag-chip input:focus { box-shadow: none; }
.tag-del { background: none; border: none; color: #c4b5fd; cursor: pointer; padding: 8px 10px; font-size: 16px; transition: color 0.2s; }
.tag-del:hover { color: #7c3aed; }
.tag-add { display: flex; align-items: center; gap: 5px; padding: 8px 14px; background: rgba(139,92,246,0.06); border: 1.5px dashed rgba(139,92,246,0.25); border-radius: 10px; color: #8b5cf6; font-size: 13px; font-weight: 500; cursor: pointer; transition: all 0.2s; }
